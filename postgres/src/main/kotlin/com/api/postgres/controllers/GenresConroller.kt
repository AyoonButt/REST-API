...
class GenresController(
    ...
    @GetMapping("/filter")
    fun filterGenres(@RequestParam query: String): ResponseEntity<List<Genre>> {
    ...
    fun filterAvoidGenres(@RequestParam query: String): ResponseEntity<List<Genre>> {
    ...
    @PostMapping("/add")
    fun addGenre(@RequestBody genre: Genre): ResponseEntity<String> {
        genresService.addGenre(genre)
        return ResponseEntity.status(HttpStatus.CREATED).body("Genre added successfully: ${genre.name}")
    }
